...
async function generateMassiveExcel() {
    ...
    const sheet = workbook.addWorksheet('Selenium_Test_Cases');
    ...
    const statuses = ['PASS', 'PASS', 'PASS', 'FAIL']; // 75% pass rate for realism
    ...
    for (let m = 0; m < modules.length; m++) {
        ...
        for (let i = 1; i <= 30; i++) {
            ...
            const row = sheet.addRow({
                id: `TC_WEB_${testId.toString().padStart(3, '0')}`,
                module: modName,
                description: `Verify functionality ${i} in ${modName} module under normal load`,
                status: status,
                time: `${(Math.random() * 2 + 0.5).toFixed(2)}s`,
                comments: status === 'FAIL' ? 'Element not found or timeout occurred.' : 'Executed successfully.'
            });
    ...
    const exportPath = path.join(__dirname, 'Selenium_300_TestCases.xlsx');
    await workbook.xlsx.writeFile(exportPath);
    console.log(`Successfully generated 300 Selenium test cases at ${exportPath}`);
}
